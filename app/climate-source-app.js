...
var generate_button = ui.Button('Sample points', function() {
  ...
  panel_right_chart.add(ui.Label((daymet_dict[key_daymet][1]).toFixed(1) + ' _____________ 0 _____________ ' + (daymet_dict[key_daymet][2]).toFixed(1)));
  ...
  var img_thumb = ui.Thumbnail(ee.Image.pixelLonLat().select(0)
  .clip(ee.Geometry.Rectangle({coords: [[era5_dict[key_era5][1], 0], [era5_dict[key_era5][2], 7]], geodesic: false}))
  .visualize({min: era5_dict[key_era5][1], max: era5_dict[key_era5][2], palette: era5_dict[key_era5][3]}));
  panel_left_chart.add(ui.Label((era5_dict[key_era5][1]).toFixed(1) + ' _____________ 0 _____________ ' + (era5_dict[key_era5][2]).toFixed(1)));
