    ...
    return ee.Image(antecedent_list.map(function(antecedent_period) {
        ...
        return percentile_list.map(function(percentile) {
          ...
          return index_list.map(function(index) {
            // Set up band names
            var lt_band = 'CMI_ante' + antecedent_period + '_lt_p' + percentile;
            ...
            var drought_mask = percent_mask.select(lt_band).eq(1);
            var baseline_mask = percent_mask.select(lt_band).eq(0);
