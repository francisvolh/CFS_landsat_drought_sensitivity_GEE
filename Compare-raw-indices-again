...
vegL = landsatprep.aggregateY(vegL);
vegL = vegL.select(['NDVI_mean', 'EVI_mean', 'NBR_mean'], ['NDVI', 'EVI', 'NBR']);
...
vegM = modisprep.aggregateY(vegM);
vegM = vegM.select(['NDVI_mean', 'EVI_mean', 'NBR_mean'], ['NDVI', 'EVI', 'NBR']);
...
var y = index + ' Landsat ' + yr;

var modis_filt = vegM.filter(ee.Filter.eq('year', yr))
                     .select([index], [x]).first();
var landsat_filt = vegL.filter(ee.Filter.eq('year', yr))
                       .select([index], [y]).first();
var img = ee.Image([modis_filt, landsat_filt]);
